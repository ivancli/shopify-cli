...
describe Script::Forms::Connect do
  ...
  describe ".ask" do
    ...
    describe "when env is not yet valid" do
      ...
      def expect_form_with_data
        form = subject
        assert_equal selected_api_key, form.app["apiKey"]
        assert_equal selected_secret, form.app["apiSecretKeys"].first["secret"]

        if selected_uuid.nil?
          assert_nil form.uuid
        else
          assert_equal selected_uuid, form.uuid
        end
      end
